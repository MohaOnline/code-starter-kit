<?php
if ( ! class_exists( 'Random_News_Grid_Posts_Widget' ) ) {
	/**
	 * Adds Random News Grid Posts Widget.
	 */
	class Random_News_Grid_Posts_Widget extends WP_Widget {
		...
		public function __construct() {
			$grid_posts_widget = array(
				...
				'description' => __( 'Retrive Grid Posts Widgets', 'random-news' ),
			);
			parent::__construct(
				'random_news_grid_posts_widget',
				__( 'Random News: Grid Posts Widget', 'random-news' ),
				$grid_posts_widget
			);
		}
		...
		public function widget( $args, $instance ) {
			...
			$read_more_button_label = ( ! empty( $instance['read_more_button_label'] ) ) ? $instance['read_more_button_label'] : __( 'Read More', 'random-news' );
			$grid_posts_post_count  = isset( $instance['number'] ) ? absint( $instance['number'] ) : 6;
		...
		public function form( $instance ) {
			...
			$read_more_button_label = isset( $instance['read_more_button_label'] ) ? $instance['read_more_button_label'] : __( 'Read More', 'random-news' );
			$grid_posts_post_count  = isset( $instance['number'] ) ? absint( $instance['number'] ) : 6;
			$grid_posts_post_offset = isset( $instance['offset'] ) ? absint( $instance['offset'] ) : '';
			$grid_posts_category    = isset( $instance['category'] ) ? absint( $instance['category'] ) : '';
			?>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'read_more_button_label' ) ); ?>"><?php esc_html_e( 'Read More Button Label:', 'random-news' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e( 'Number of posts to show( Min: 1 | Max: 6 ):', 'random-news' ); ?></label>
