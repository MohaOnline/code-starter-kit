...
class PostExcerptField extends CustomField
{
    ...
    public function getTwigArgs(
        ...
        if (!$currentPost ||
            !post_type_supports($currentPost->post_type, 'excerpt')) {
            return $args;
        }

        // custom modification to avoid issues (see body for the details)
        $excerpt = $this->getExcerpt($currentPost->post_excerpt, $currentPost);
