	...
	function init( $elem ) {
		...
		var init_player = function() {
			...
			player.ready(function() {
				...
				$elem.find( '.vjs-big-play-button' ).one( 'click', function() {
					if ( ! video_has_started ) {
						$elem.addClass( 'vjs-waiting' );
					}
				});				
			});
			...
			if ( settings.hasOwnProperty( 'logo' ) ) {
				...
				var logo = '<img ' +  merge_attributes( attributes ) + ' alt="" />';
		...
		if ( settings.cookie_consent ) {
			...
			$elem.on( 'cookieConsent', function( event, args ) {
				if ( $elem.find( '.aiovg-privacy-wrapper' ).length > 0 ) {
					init_player();
					$elem.find( '.aiovg-privacy-wrapper' ).remove();
				}
			});
