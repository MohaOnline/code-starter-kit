<style type="text/css">
    ...
    #tsvgColorBox<?php echo esc_attr( $tsvg_shortcode_id ); ?><?php echo esc_attr( $tsvg_shortcode_id ); ?>, #tsvgParallaxColorBoxContent<?php echo esc_attr( $tsvg_shortcode_id ); ?>, #tsvgParallaxColorBoxLoadedContent<?php echo esc_attr( $tsvg_shortcode_id ); ?> {
    ...
    #tsvgColorBox<?php echo esc_attr( $tsvg_shortcode_id ); ?><?php echo esc_attr( $tsvg_shortcode_id ); ?> {
        outline: 0;
    }
    ...
    #tsvgColorBox<?php echo esc_attr( $tsvg_shortcode_id ); ?><?php echo esc_attr( $tsvg_shortcode_id ); ?>, #tsvgParallaxColorBoxOverlay<?php echo esc_attr( $tsvg_shortcode_id ); ?>, #tsvgParallaxColorBoxWrapper<?php echo esc_attr( $tsvg_shortcode_id ); ?> {
        overflow: visible !important;
        max-width: 100% !important;
    }
    #tsvgParallaxColorBoxContent<?php echo esc_attr( $tsvg_shortcode_id ); ?> {
        max-width: calc(100% - calc(2 * var(--tsvg_popup_ps_b_<?php echo esc_attr( $tsvg_shortcode_id ); ?>)));
    }
...
<script type="text/javascript">
        ...
        $.tsvgParallaxSwipeBox<?php echo esc_attr( $tsvg_shortcode_id ); ?> = function (elem, options) {
            ...
            var ui = {
                init: function (index) {
                    ...
                    return '<div class="tsvg-parallax-swipebox-<?php echo esc_attr( $tsvg_shortcode_id ); ?>-video-container" style="max-width:' + plugin.settings.videomaxWidth + 'px"><div class="tsvg-parallax-swipebox-<?php echo esc_attr( $tsvg_shortcode_id ); ?>-video"   style="height:' + url_heigth + 'px; ">' + iframe + "</div></div>"
    ...
    jQuery(document).ready(function() {
        if(tsvgParallaxPopupType<?php echo esc_attr( $tsvg_shortcode_id ); ?>==1) {
            ...
            jQuery('.tsvg-parallax-block-swipebox-<?php echo esc_attr( $tsvg_shortcode_id ); ?>').tsvgColorBox<?php echo esc_attr( $tsvg_shortcode_id ); ?>({
                iframe: true, transition: tsvgParallaxPopupTransition<?php echo esc_attr( $tsvg_shortcode_id ); ?>, innerWidth: jQuery(window).width() * 0.7, innerHeight: jQuery(window).width() * 0.7 * 0.6, maxWidth: "80%", maxHeight: "80%", current: "", rel: 'slideshow', slideshow: false
            });
        }
    });
    <?php if ( $tsvg_edit === 'true' ) { ?>
		window.tsvgParalaxEngine = function(setType,setTransition){
            if (setType == 1) {
                ...
                jQuery(".tsvg-parallax-block-swipebox-<?php echo esc_attr( $tsvg_shortcode_id ); ?>").tsvgColorBox<?php echo esc_attr( $tsvg_shortcode_id ); ?>(
					{
						iframe: true, transition: setTransition, innerWidth: jQuery(window).width() * 0.7, innerHeight: jQuery(window).width() * 0.7 * 0.6, maxWidth: "80%", maxHeight: "80%", current: "", rel: 'slideshow', slideshow: false
					}
				);
            }
		}
	<?php } ?>
    jQuery(document).on('mousemove', '.tsvg-parallax-block-<?php echo esc_attr($tsvg_shortcode_id); ?>',function (event) {
        event=event || window.event
        let tsvgImgEffect<?php echo esc_attr( $tsvg_shortcode_id ); ?> = jQuery(this).attr("data-tsvg-ef"),
            tsvgTitleEffect<?php echo esc_attr( $tsvg_shortcode_id ); ?> = jQuery(this).find('.tsvg-parallax-block-title-effect').attr("data-tsvg-ef"),
            tsvgLineEffect<?php echo esc_attr( $tsvg_shortcode_id ); ?> = jQuery(this).find('.mask').attr("data-tsvg-ef");
