...
class TS_Video_Gallery {
	...
	public function tsvg_activate_scripts() {
		...
		wp_register_style( "tsvg-special", plugin_dir_url( __DIR__ ) . 'public/css/tsvg-content-special.css', array(),$this->version, 'all' );
	...
	private function tsvg_shortcode_content( $tsvg_shortcode_id, $tsvg_edit ) {
		...
		wp_enqueue_style("tsvg-special-{$tsvg_shortcode_id}", plugin_dir_url( __DIR__ ) . 'public/css/tsvg-content-special.css', array(), time(), 'all' );
		...
		echo sprintf(
			'<section class="tsvg-section-%1$s"  id="tsvg-section-%1$s">',
