(function ($) {
	'use strict';
	let tsvgResizeContentPreview,
		tsvgToastrOptions = { "closeButton": true, "newestOnTop": false, "progressBar": true, "positionClass": "toast-top-right", "preventDuplicates": true, "showDuration": "300", "hideDuration": "1000", "timeOut": "5000", "extendedTimeOut": "1000", "showEasing": "swing", "hideEasing": "linear", "showMethod": "fadeIn", "hideMethod": "fadeOut", },
		tsvgThemeSettings = [],
		tsvgThemeOptionStyle = [];
	...
	if (typeof tsvg_builder_object !== 'undefined') {
		let tsvgOptions = JSON.parse(tsvg_builder_object.tsvg_proporties.TS_VG_Option);
		tsvgResizeContentPreview = function () {
			switch (tsvgOptions.TS_vgallery_Q_Theme) {
	...
	$(function () {
		if (typeof tsvg_builder_object !== 'undefined') {
			...
			tsvgThemeOptionStyle = JSON.parse(tsvg_builder_object.tsvg_proporties.TS_VG_Option_Style);
			...
			let tsvgVideoProps = Object();
			Object.keys(tsvg_builder_object.tsvg_proporties.tsvg_video_records).map((key) => tsvgVideoProps[key] = tsvg_builder_object.tsvg_proporties.tsvg_video_records[key]);
			...
			function tsvgSetImage(type, id, url, width, height) {
				...
				tsvgVideoProps[`${tsvgEditId}`].TS_VG_Options.TotalSoftVGallery_Vid_Im = url;
				$(`main.tsvg-main-content-${tsvgShortcodeId}  ul li[data-tsvg-id='${tsvgEditId}']  img`).not('.emoji').attr("src", tsvgVideoProps[`${tsvgEditId}`].TS_VG_Options.TotalSoftVGallery_Vid_Im);
				if (tsvgShortcodeTheme == 'Elastic Gallery') {
					$(`main.tsvg-main-content-${tsvgShortcodeId}  li[data-tsvg-id='${tsvgEditId}']`).attr("data-poster", tsvgVideoProps[`${tsvgEditId}`].TS_VG_Options.TotalSoftVGallery_Vid_Im);
			...
			function tsvgAddNewVideo(tsvgAction, tsvgTitle, tsvgCopiedId = "") {
				let tsvgNewBlockId = `new-${Math.floor(Math.random() * 900000) + 100000}`,
					tsvgNewBlock = "",
					tsvgBlockVideoUrl = "",
					tsvgTransitionDelay = 0.3 * $(`main.tsvg-main-content-${tsvgShortcodeId}  li`).length,
					tsvgBlockTitle = tsvgTitle,
					tsvgBlockImgUrl = tsvgAction == "add" ? tsvg_builder_object.tsvg_no_img : $(`main.tsvg-main-content-${tsvgShortcodeId}  li[data-tsvg-id='${tsvgCopiedId}'] img`).attr("src"),
					tsvgNewPopupBlock = "";
				if (tsvgAction == "copy") {
					let tsvgNewBlockProps = JSON.parse(JSON.stringify(tsvgVideoProps[tsvgCopiedId]));
					tsvgNewBlockProps.id = `${tsvgNewBlockId}`;
					tsvgVideoProps[`${tsvgNewBlockId}`] = tsvgNewBlockProps;
					$(`.tsvg-list-item[aria-tsvg-video="${tsvgCopiedId}"]`).clone(true).attr("aria-tsvg-video", tsvgNewBlockId).appendTo("#tsvg-list");
				...
				switch (tsvgShortcodeTheme) {
					case 'Grid Video Gallery':
						...
						tsvgNewBlock = `
							<li class="tsvg-grid-layout-item tsvg-grid-layout-item-${tsvgShortcodeId}"  data-tsvg-id='${tsvgNewBlockId}' style='-moz-animation-delay: ${tsvgTransitionDelay}s;-webkit-animation-delay: ${tsvgTransitionDelay}s;animation-delay: ${tsvgTransitionDelay}s;' >
								<figure  class="tsvg-grid-layout-item-block-${tsvgShortcodeId}" data-tsvg-effect="${tsvgGridEffectSvg}" data-tsvg-bool="${tsvgGridBool}" >
									...
									<h3 class="tsvg-grid-layout-item-title-${tsvgShortcodeId}"  data-tsvg-title="${tsvgGridTitle}">
										${tsvgBlockTitle}
									</h3>
									<figcaption class="tsvg-grid-layout-item-caption-${tsvgShortcodeId}" >
										${tsvgGridDesc}
									</figcaption>
								</figure>
							</li> 
						...
						`;
						$(tsvgNewPopupBlock).insertAfter($(`figure .tsvg-grid-slides-${tsvgShortcodeId} > li`).last());
						break;
				...
				$(tsvgNewBlock).insertAfter($(`main.tsvg-main-content-${tsvgShortcodeId} li`).last());
					...
					jQuery(`.tsvg-thumbnails-block-img-${tsvgShortcodeId}`).adipoli(
						{
							...
							'popOutShadowC': $("#TotalSoft_GV_1_10").val() * 1,
						}
					);
			...
			$(document).on(
				...
				function () {
					...
					if ($(tsvg_selected_icon).length != 0) {
						...
						} else if (tsvgThemeOptionStyle.hasOwnProperty(`${tsvg_select_item_for}`)) {
							tsvgThemeOptionStyle[tsvg_select_item_for] = tsvg_selected_value;
						}
					} else {
			...
			$(document).on(
				...
				function () {
					...
					} else if (tsvgThemeOptionStyle.hasOwnProperty(`${tsvg_selection_field}`)) {
						tsvgThemeOptionStyle[tsvg_selection_field] = tsvg_selected_value;
					}
					tsvgSetIconClass($(tsvg_input_icon_value).attr("data-tsvg-elem"), $(tsvg_input_icon_value).attr("data-change-prop"), tsvg_selected_value);
				}
			);
			var tsvgMutationObserver = new MutationObserver(
				function (mutations) {
					mutations.forEach(
						function (mutation) {
							if (tsvgVideoProps.hasOwnProperty(`${tsvgEditId}`) && tsvgGetTextareaContext('tsvg_content_area') != '') {
								tsvgVideoProps[`${tsvgEditId}`].TS_VG_Options.TotalSoftVGallery_Vid_desc = tsvgGetTextareaContext('tsvg_content_area');
								switch (tsvgShortcodeTheme) {
									case 'Grid Video Gallery':
			...
			$(window).load(
				function () {
					...
					jQuery(jQuery('[id="tsvg_content_area_ifr"]')[0].contentWindow.document).on(
						'keyup',
						function (e) {
							if (tsvgVideoProps.hasOwnProperty(`${tsvgEditId}`)) {
								tsvgVideoProps[`${tsvgEditId}`].TS_VG_Options.TotalSoftVGallery_Vid_desc = tsvgGetTextareaContext('tsvg_content_area');
								switch (tsvgShortcodeTheme) {
									case 'Grid Video Gallery':
										$(`main.tsvg-main-content-${tsvgShortcodeId}  .tsvg-grid-layout-item[data-tsvg-id='${tsvgEditId}'] figcaption`).html(`${tsvgGetTextareaContext('tsvg_content_area')}`);
			...
			$('#wp-tsvg_content_area-editor-container .mce-top-part ').on(
				...
				function () {
					$(`main.tsvg-main-content-${tsvgShortcodeId}  .tsvg-grid-layout-item[data-tsvg-id='${tsvgEditId}'] figcaption`).html(`${tsvgGetTextareaContext('tsvg_content_area')}`);
					if (tsvgVideoProps[`${tsvgEditId}`].hasOwnProperty(`TotalSoftVGallery_Vid_desc`)) {
						tsvgVideoProps[`${tsvgEditId}`].TS_VG_Options.TotalSoftVGallery_Vid_desc = tsvgGetTextareaContext('tsvg_content_area');
					}
				}
			)
			...
			$(`#tsvg_video_color`).spectrum(
				{
					...
					move: function (color) {
						...
						if (tsvgVideoProps[`${tsvgEditId}`].TS_VG_Options.hasOwnProperty(`TotalSoftVGallery_Vid_Cl`)) {
							tsvgVideoProps[`${tsvgEditId}`].TS_VG_Options.TotalSoftVGallery_Vid_Cl = color.toRgbString();
						}
					}
				}
			);
			$(document).on(
				...
				function () {
					tsvgVideoProps[`${tsvgEditId}`].TS_VG_Options.TotalSoftVGallery_Vid_link = $(this).val();
					switch (tsvgShortcodeTheme) {
						case 'Grid Video Gallery':
			...
			$(document).on(
				...
				function () {
					tsvgVideoProps[`${tsvgEditId}`].TS_VG_Options.TotalSoftVGallery_Vid_vont = `${$(this).is(':checked')}`;
					switch (tsvgShortcodeTheme) {
						case 'Grid Video Gallery':
							if ($(this).is(':checked')) {
			...
			$(document).on(
				...
				function () {
					if (tsvgVideoProps[`${tsvgEditId}`].hasOwnProperty(`TS_VG_SetName`)) {
						tsvgVideoProps[`${tsvgEditId}`].TS_VG_SetName = $(this).val();
					}
					switch (tsvgShortcodeTheme) {
						case 'Grid Video Gallery':
			...
			$(document).on(
				...
				function () {
					...
					$(this).parent().remove();
					delete tsvgVideoProps[tsvg_delete_video_id];
			...
			$(document).on(
				...
				function () {
					...
					$("input#tsvg_TS_VG_SetName").val(tsvgVideoProps[`${tsvgEditId}`].TS_VG_SetName);
					$("input#tsvg_video_link").val(tsvgVideoProps[`${tsvgEditId}`].TS_VG_Options.TotalSoftVGallery_Vid_link);
					$("input#tsvg_video_Video_ONT").prop('checked', false);
					if (tsvgVideoProps[`${tsvgEditId}`].TS_VG_Options.TotalSoftVGallery_Vid_vont) {
						$("input#tsvg_video_Video_ONT").prop('checked', true);
					}
					if (tsvgVideoProps[`${tsvgEditId}`].TS_VG_Options.TotalSoftVGallery_Vid_desc) {
						tsvgSetTextareaContext(
							tsvgVideoProps[`${tsvgEditId}`].TS_VG_Options.TotalSoftVGallery_Vid_desc,
							'tsvg_content_area'
						);
					}
					if (tsvgVideoProps[`${tsvgEditId}`].TS_VG_Options.TotalSoftVGallery_Vid_Im == "") {
						...
						$.ajax(
							{
								...
								data: {
									...
									attachment_url: tsvgVideoProps[`${tsvgEditId}`].TS_VG_Options.TotalSoftVGallery_Vid_Im
								},
								beforeSend: function () {
					...
					if (tsvgVideoProps[`${tsvgEditId}`].TS_VG_Options.TotalSoftVGallery_Vid_Vd == "") {
						...
						let url = tsvgVideoProps[`${tsvgEditId}`].TS_VG_Options.TotalSoftVGallery_Vid_Vd;
					...
					$(`#${$(this).attr('id')}`).spectrum(
						{
							...
							move: function (color) {
								...
								} else if (tsvgThemeOptionStyle.hasOwnProperty(`${tsvg_spectrum_element_id}`)) {
									tsvgThemeOptionStyle[tsvg_spectrum_element_id] = color.toRgbString();
								}
			...
			$(document).on(
				...
				function (event) {
					...
					} else if (tsvgThemeOptionStyle.hasOwnProperty(`${`${tsvg_change_style}`}`)) {
						tsvgThemeOptionStyle[`${tsvg_change_style}`] = $(this).attr('data-tsvg-pos');
					}
			...
			$(document).on(
				...
				function (event) {
					...
					if (tsvgStyles.hasOwnProperty(`${tsvg_change_style}`)) {
						...
						tsvgThemeOptionStyle[`${tsvg_change_style}`] = event.target.value;
					}
			...
			$(document).on(
				...
				function (event) {
					...
					} else if (tsvgThemeOptionStyle.hasOwnProperty(`${$(this).attr("id")}`)) {
						tsvgThemeOptionStyle[`${$(this).attr("id")}`] = `${$(this).is(':checked')}`;
					}
					if ($(this).is(':checked')) {
			...
			$(document).on(
				...
				function (event) {
					if (tsvgStyles.hasOwnProperty(`${$(this).attr("id")}`)) {
						...
						tsvgThemeOptionStyle[`${$(this).attr("id")}`] = `${event.target.value}`;
					}
					...
					$.ajax(
						{
							url: tsvg_builder_object.ajaxurl,
							data: {
								action: 'tsvg_save_record',
								tsvg_nonce: tsvg_builder_object.tsvg_nonce,
								tsvg_id: tsvgShortcodeId,
								tsvg_TS_VG_Title: tsvg_builder_object.tsvg_proporties.TS_VG_Title,
								tsvg_videos: tsvgVideoProps,
								tsvg_TS_VG_Sort: tsvg_video_sort_array,
								tsvg_TS_VG_Styles: tsvgStyles,
								tsvg_TS_VG_Options: tsvgOptions,
								tsvg_TS_VG_Settings: tsvgThemeSettings,
								tsvg_TS_VG_Option_Style: tsvgThemeOptionStyle,
								tsvg_deleted_videos: tsvgDeleteIds,
							},
							beforeSend: function () {
						...
						function (response) {
							if (response.success === true) {
								if (response.data.hasOwnProperty("url")) {
									...
									toastr["error"](`Your vgallery isn't saved.`, 'Error', tsvgToastrOptions)
									setTimeout(
										() => {
				...
				function (e) {
					...
					tsvg_wp_media_library.state('embed').on(
						...
						function () {
							...
							if (url.indexOf('youtube.com/shorts/') > -1 && url.indexOf('?') > -1) {
								let tsvgShortsSplit = url.split("?");
								url = tsvgShortsSplit[0];
							}
							wp.apiRequest(
								{
									url: wp.media.view.settings.oEmbedProxyUrl,
								...
								function (response) {
														...
														getYTMaxResolution(
															...
															(width, height) => {
																if (width > 0) {
																	...
																	tsvgSetImage("embed", "", tsvgImgSrc.replace('maxresdefault.jpg', 'hqdefault.jpg'), tsvgImgW, tsvgImgH);
																}
															}
														);
									...
									tsvgVideoProps[`${tsvgEditId}`].TS_VG_Options.TotalSoftVGallery_Vid_Vd = url;
									if (tsvgShortcodeTheme == 'Grid Video Gallery') {
									...
									if (tsvgShortcodeTheme == 'Space Gallery') {
										$(`main.tsvg-main-content-${tsvgShortcodeId}  li[data-tsvg-id='${tsvgEditId}']`).attr("data-tsvg-src", url);
										$(`main.tsvg-main-content-${tsvgShortcodeId}  li[data-tsvg-id='${tsvgEditId}'] .tsvg-video-wrapper `).html(` <iframe src="${url}" frameborder = "0" webkitAllowFullScreen > </iframe> `)
									}
								}
							).fail(
								function () {
