    ...
    /** @lends Kekule.Widget.BaseWidget# */
    {
      ...
      initProperties: function () {
        ...
        this.defineProp('observeElementAttribChanges', {
          'dataType': DataType.BOOL, // 'scope': Class.PropertyScope.PUBLIC,
          'setter': function (value) {
        ...
        this.defineProp('htmlClassName', {
          ...
          'setter': function (value) {
            this.getElement().className = value;
          }
        });
        ...
        this.defineProp('displayed', {
          'dataType': DataType.BOOL,
          'serializable': false,
          'getter': function () { return Kekule.StyleUtils.isDisplayed(this.getElement()); },
          'setter': function (value, byPassShowStateChange) {
            Kekule.StyleUtils.setDisplay(this.getElement(), value); if (!byPassShowStateChange) { this.widgetShowStateChanged(this.isShown()); }
          }
        });
        ...
        this.defineProp('useCornerDecoration', {
          ...
          'setter': function (value) {
            ...
            if (!value) { this.removeClassName(CNS.CORNER_ALL); } else { this.addClassName(CNS.CORNER_ALL); }
          }
        });
      ...
      /* 底层修改 */// 修改按钮的背景图片
      setBgPic (state) {
        if (this.CLASS_NAME == 'Kekule.Widget.RadioButton') {
          var parentHtmlName = this.getParent().getHtmlClassName();
          if (parentHtmlName.indexOf('K-Chem-Composer-Common-Toolbar') != -1) {
          } else if (parentHtmlName.indexOf('K-Chem-Composer-Chem-Toolbar') != -1) {
            if (this.getElement().childNodes.length == 3) {
              var radioButFirstNode = this.getElement().childNodes[0];
              var mClassNames = radioButFirstNode.className;
              if (state != 'Normal' && mClassNames && mClassNames.indexOf('icon-') != -1) {
                var classAry = mClassNames.split(' ');
                var newClassNames = '';
                for (var k = 0; k < classAry.length; k++) {
                  if (classAry[k].indexOf('icon-') != -1 && classAry[k].indexOf('_select') == -1) {
                    classAry[k] += '_select';
                  }
                  // newClassNames += (" "+classAry[k]);
                  newClassNames += (newClassNames == '' ? classAry[k] : (' ' + classAry[k]));
                }
                radioButFirstNode.className = newClassNames;
              } else if (state == 'Normal' && mClassNames && mClassNames.indexOf('_select') != -1) {
                var classAry = mClassNames.split(' ');
                var newClassNames = '';
                for (var k = 0; k < classAry.length; k++) {
                  if (classAry[k].indexOf('icon-') != -1 && classAry[k].indexOf('_select') != -1) {
                    classAry[k] = classAry[k].substring(0, classAry[k].length - 7);
                  }
                  newClassNames += (newClassNames == '' ? classAry[k] : (' ' + classAry[k]));
                }
                radioButFirstNode.className = newClassNames;
          ...
          } else if (parentHtmlName.indexOf('K-Chem-Composer-Assoc-Toolbar') != -1) {
            if (this.getElement().childNodes.length == 3) {
              var radioButFirstNode = this.getElement().childNodes[0];
              var mClassNames = radioButFirstNode.className;
              if (state != 'Normal' && mClassNames && mClassNames.indexOf('icon-') != -1) {
                ...
                for (var k = 0; k < classAry.length; k++) {
                  if (classAry[k].indexOf('icon-') != -1 && classAry[k].indexOf('_select') == -1) {
                    var sub = classAry[k].substring(5);
                    sub = sub.substring(0, sub.length - 1) + '_select' + sub.substring(sub.length - 1);
                    classAry[k] = 'icon-' + sub;
                  }
                  newClassNames += (newClassNames == '' ? classAry[k] : (' ' + classAry[k]));
                }
                ...
                for (var k = 0; k < classAry.length; k++) {
                  if (classAry[k].indexOf('icon-') != -1 && classAry[k].indexOf('_select') != -1) {
                    var tempIndex = classAry[k].indexOf('_select');
                    classAry[k] = classAry[k].substring(0, tempIndex) + classAry[k].substring(classAry[k].length - 1);
                  }
                  newClassNames += (newClassNames == '' ? classAry[k] : (' ' + classAry[k]));
                }
                radioButFirstNode.className = newClassNames;
      ...
      defineElemAttribMappingProp: function (propName, elemAttribName, options) {
        var ops = Object.extend({
          ...
          'serializable': false,
          'getter': function () {
            return this.getElement() && this.getElement().getAttribute(elemAttribName);
          },
          'setter': function (value) {
            this.getElement() && this.getElement().setAttribute(elemAttribName, value);
          }
        }, options || {});
      ...
      defineElemStyleMappingProp: function (propName, stylePropName, options) {
        var ops = Object.extend({
          ...
          'serializable': false,
          'getter': function () {
            return this.getStyleProperty(stylePropName);
          },
          'setter': function (value) {
            this.setStyleProperty(stylePropName, value);
          }
        }, options || {});
        return this.defineProp(propName, ops);
      },
      ...
      stateChanged: function () {
        ...
        /* 底层修改 */
        if (this._stateClassName == CNS.STATE_NORMAL) {
          this.setBgPic('Normal');
        } else {
          this.setBgPic('Active');
        }
        // notify children
       ...
       */
      getPicName: function (name) {
        var filterBond = {
          'K-Chem-MolBondIaController-Single': 'icon-kBondA',
          'K-Chem-MolBondIaController-SingleCustom': 'icon-kBondO',
          'K-Chem-RepositoryStructureFragmentIaController-Fischer1Custom': 'icon-kBondP',
          'K-Chem-RepositoryStructureFragmentIaController-Fischer2Custom': 'icon-kBondQ',
          'K-Chem-RepositoryStructureFragmentIaController-Fischer3Custom': 'icon-kBondR',
          'K-Chem-MolBondIaController-Double': 'icon-kBondB',
          'K-Chem-MolBondIaController-Triple': 'icon-kBondC',
          'K-Chem-MolBondIaController-Closer': 'icon-kBondM',
          'K-Chem-MolBondIaController-WedgeUp': 'icon-kBondG',
          'K-Chem-MolBondIaController-WedgeDown': 'icon-kBondH',
          'K-Chem-MolBondIaController-WedgeUpOrDown': 'icon-kBondK',
          'K-Chem-MolBondIaController-Double-Either': 'icon-kBondN',
          'K-Chem-RepositoryStructureFragmentIaController-Methane': 'icon-kBondD',
          'K-Chem-RepositoryStructureFragmentIaController-Fischer1': 'icon-kBondE',
          'K-Chem-RepositoryStructureFragmentIaController-Fischer2': 'icon-kBondF',
          'K-Chem-RepositoryStructureFragmentIaController-Fischer3': 'icon-kBondL',
          'K-Chem-RepositoryStructureFragmentIaController-SawhorseStaggered': 'icon-kBondI',
          'K-Chem-RepositoryStructureFragmentIaController-SawhorseEclipsed': 'icon-kBondJ'
        };
        var filterRing = {
          'K-Chem-MolRingIaController-3': 'icon-kRingA',
          'K-Chem-MolRingIaController-4': 'icon-kRingB',
          'K-Chem-MolRingIaController-5': 'icon-kRingC',
          'K-Chem-MolRingIaController-6': 'icon-kRingD',
          'K-Chem-MolRingIaController-7': 'icon-kRingE',
          'K-Chem-MolRingIaController-8': 'icon-kRingF',
          'K-Chem-MolRingIaController-Ar-6': 'icon-kRingG',
          'K-Chem-MolRingIaController-Ar-61': 'icon-kRingO',
          'K-Chem-MolRingIaController-Ar-5': 'icon-kRingH',
          'K-Chem-RepositoryStructureFragmentIaController-CyclopentaneHaworth1': 'icon-kRingI',
          'K-Chem-RepositoryStructureFragmentIaController-CyclopentaneHaworth2': 'icon-kRingJ',
          'K-Chem-RepositoryStructureFragmentIaController-CyclohexaneHaworth1': 'icon-kRingK',
          'K-Chem-RepositoryStructureFragmentIaController-CyclohexaneHaworth2': 'icon-kRingL',
          'K-Chem-RepositoryStructureFragmentIaController-CyclohexaneChair1': 'icon-kRingM',
          'K-Chem-RepositoryStructureFragmentIaController-CyclohexaneChair2': 'icon-kRingN'
        };
        var filterArrow = {
          'K-Chem-ArrowLineIaController-OpenArrowLine': 'icon-kArrowA',
          'K-Chem-ArrowLineIaController-TriangleArrowLine': 'icon-kArrowT',
          'K-Chem-ArrowLineIaController-ReversibleArrowLine': 'icon-kArrowD',
          'K-Chem-ArrowLineIaController-Line': 'icon-kArrowL',
          'K-Chem-ArrowLineIaController-HeatSymbol': 'icon-kArrowH',
          'K-Chem-ArrowLineIaController-BracketSymbol': 'icon-kBracketA',
          'K-Chem-ArrowLineIaController-AddSymbol': 'icon-kArrowAD'
        };
        if (filterBond[name]) {
          return filterBond[name];
        } else if (filterRing[name]) {
          return filterRing[name];
        } else if (filterArrow[name]) {
          return filterArrow[name];
        } else {
          return undefined;
        }
      },
      addClassName: function (classNames, affectCustomProp) {
        if (this.getElement()) {
          if (affectCustomProp) {
            ...
            // 底层修改
            var filterNameAry = ['K-Chem-Undo', 'K-Chem-Redo', 'K-Chem-Copy', 'K-Chem-MolBondIaController',
              'K-Chem-MolAtomIaController', 'K-Chem-MolRingIaController', 'K-Chem-BasicMolManipulationIaController', 'K-Chem-BasicMolEraserIaController',
              'K-Chem-FormulaIaController', 'K-Chem-ArrowLineIaController'];

            function returnLegalClassName (name) {
              var needNameAry = ['Undo', 'Redo', 'Copy', 'Bond', 'Atom', 'Ring', 'Manipulation', 'Eraser', 'Formula', 'ArrowLine'];
              for (var i = 0; i < needNameAry.length; i++) {
                var mIndex = name.indexOf(needNameAry[i]);
                if (mIndex != -1) {
                  if (needNameAry[i] == 'Bond' || needNameAry[i] == 'Ring') {
                    return ' icon-k' + needNameAry[i] + 'But';
                  }
                  return ' icon-k' + needNameAry[i];
                }
              }
            }

            var filterIndex = filterNameAry.indexOf(cname);
            var but = this.getElement();
            var firstSpan = but.childNodes[0];
            if (filterIndex != -1 && firstSpan) {
              firstSpan.className += returnLegalClassName(cname);
            } else if (this.getPicName(cname) && firstSpan) {
              firstSpan.className += ' ' + this.getPicName(cname);
            } else {
              this.setCustomHtmlClassName(cname);
            }
          } else {
      ...
      elementChanged: function (newElem, oldElem) {
        if (oldElem) { this.unbindElement(oldElem); } // uninstall event handlers
        if (newElem) { this.bindElement(newElem); }
      },
      /**
      ...
      reactUiEvent: function (e) {
        ...
        if (this.getStatic()) { // static, do nothing
          ...
          if (evType === 'mousemove') { // test mouse cursor
            ...
            if (eventOnSelf && !e.ghostMouseEvent) { this.reactActiviting(e); }
            handled = true;
          } else if ((evType === 'mouseup' && e.getButton() === Kekule.X.Event.MouseButton.LEFT) || (evType === 'touchend') || (evType === 'touchcancel')) {
            if (eventOnSelf && !e.ghostMouseEvent) {
              if (e.target.tagName == 'INPUT' && e.target.type == 'range') {
                  // 画布的缩小 放大 slider 自定义   底层修改
              } else {
                this.reactDeactiviting(e);
              }
            }
      ...
      getEventMouseRelCoord: function (e, relElement) {
        if (!relElement) {
          relElement = this.getCoreElement();
        }  // defaultly base on client element, not widget element
        var coord = { 'x': e.getClientX(), 'y': e.getClientY() };
        var offset = {
          'x': relElement.getBoundingClientRect().left - relElement.scrollLeft,
          'y': relElement.getBoundingClientRect().top - relElement.scrollTop
        };
  ...
  Kekule.Widget.InteractionController = Class.create(ObjectEx,
    /** @lends Kekule.Widget.InteractionController# */
    {
      ...
      _getEventMouseCoord: function (e, clientElem) {
        ...
        var offset = {
          'x': elem.getBoundingClientRect().left - elem.scrollLeft,
          'y': elem.getBoundingClientRect().top - elem.scrollTop
        };
        var result = Kekule.CoordUtils.substract(coord, offset);
        return result;
      }
  ...
  Kekule.Widget.GlobalManager = Class.create(ObjectEx,
    /** @lends Kekule.Widget.GlobalManager# */
    {
      ...
      reactUiEvent: function (e) {
        ...
        // check first if the component has event handler itself
        var funcName = Kekule.Widget.getEventHandleFuncName(e.getType());
        if (this[funcName]) { this[funcName](e); } // has own handler
        // dispatch to widget
        if (targetWidget) {
