...
/*底层修改*/
//自定义的滑动条
Kekule.Widget.CustomSlider = Class.create(Kekule.Widget.BaseWidget,
	/** @lends Kekule.Widget.Button# */
	{
		/** @private */
		CLASS_NAME: 'Kekule.Widget.CustomSlider',
		/** @private */
		...
		{
			return $super() ; //this.CSS_CLASS_NAME;
		},
		/** @ignore */
		doCreateRootElement: function(doc)
		{
			...
			result.setAttribute('type', 'range');  // avoid default submit type
			result.setAttribute('min',1);
			result.setAttribute('max',100);
			result.setAttribute('value',100);
			result.style.width = 64+'px';
			result.style.height = 4+'px';

			result.addEventListener('input',function(e){
				var lable = doc.getElementById('customSliderText');
				lable.innerHTML = this.value+'%';
				e.customRadio = this.value/100;
				self.doReactDeactiviting(e);
			})
			return result;
		},
...
//自定义text 用于显示百分比
Kekule.Widget.CustomText = Class.create(Kekule.Widget.BaseWidget,
	/** @lends Kekule.Widget.Button# */
	{
		/** @private */
		CLASS_NAME: 'Kekule.Widget.CustomText',
		/** @private */
		BINDABLE_TAG_NAMES: ['input', 'button', 'a', 'label'],
		/** @private */
		CSS_DROPDOWN_CLASS_NAME: 'K-DropDownMark',
		...
		doCreateRootElement: function(doc)
		{
			...
			result.setAttribute('id','customSliderText');
			result.style.width = 24+'px';
			result.style.height = 14+'px';
			result.innerHTML = '100%';
    		result.style.right = '-4px';
...
/** @lends Kekule.Widget.Button# */
{
	...
	{
		// now use an unicode triangle to mark drop down. Need to change it to image in the future.
