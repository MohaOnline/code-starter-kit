...
/** @lends Kekule.Widget.Dialog# */
{
	...
	initialize: function($super, parentOrElementOrDocument, caption, buttons)
	{
		...
		this._childButtons = [];
		this.ifPeriodic = false;//是否编写化学周期表 底层修改
		if(caption == "Periodic table"){
	...
	doCreateSubElements: function(doc, rootElem)
	{
		...
		if(this.ifPeriodic){
			...
			elem.style.top = '73px';
			var nameAry = ['非金属','惰性气体','碱金属','碱土金属','非金属','卤素','金属','过渡金属'];
			var colorAry = ['#CFD8DC','#BBDEFB','#FFF9C4','#FFE0B2','#D7CCC8','#C5CAE9','#B2DFDB','#C8E6C9'];
			for(var i = 0;i < 8;i++){
				var elem1 = doc.createElement('div');
				elem1.style.marginTop = '13px';
				elem1.style.width = '75px';elem1.style.height = '15px';
				elem1.style.float = 'left';
				elem.appendChild(elem1);
				var colorBlock = doc.createElement('div');
				colorBlock.style.borderRadius = '3px';
				colorBlock.style.width = '15px';colorBlock.style.height = '15px';
				colorBlock.style.backgroundColor = colorAry[i];
				colorBlock.style.float = 'left';
				elem1.appendChild(colorBlock);
				var fontBlock = doc.createElement('div');
				fontBlock.innerText = nameAry[i];
				fontBlock.style.float = 'left';
				fontBlock.style.marginLeft = '2px';
				fontBlock.style.lineHeight = '15px';
				fontBlock.style.fontSize = '12px';
				fontBlock.style.color = '#757575';
				elem1.appendChild(fontBlock);
			}
		}
		// client element
		var elem = doc.createElement('div');
	...
	createDialogButton: function(btnName, btnResName, doc, btnPanel)
	{
		...
		if (btnInfo)
		{
			...
			if (resName && (btnName != 'ok'&&btnName !='cancel')/*底层修改*/)
			{
				btn.linkStyleResource(resName);
			}
			this._childButtons.push(btn);
			/*底层修改*/
			if(btnName != 'ok' || btnName !='cancel' && btn){
				btn.getElement().style.width = '80px';
				btn.getElement().style.height = '32px';
				btn.getElement().style.marginRight = '20px';
				btn.getElement().style.fontSize = '16px';
				btn.getElement().style.backgroundColor = 'white';
				btn.getElement().style.color = '#757575';
				btn.getElement().style.lineHeight = '16px';
				btn.getElement().style.marginRight = '15px';
			}
			return btn;
		}
		else
