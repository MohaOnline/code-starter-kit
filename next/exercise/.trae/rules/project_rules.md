Key Principles
- Generate and update markdown document to trace what is planned and what is done for the request, file could be saved to sub folder 'docs' in working folder (where code is generated or updated) with "changelog-number" (like: changelog-001.md, changelog-002.md, etc) naming custom.
- For further work, refer to `changelog-001.md` and `changelog-002.md` documents generated by above step if existing for details besides code as well.
